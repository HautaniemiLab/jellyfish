...
class DataAnalyzer:
    ...
    def calc_all_clonal_freqs(self):
        ...
        for file in self.files:
            ...
            freqs = freqs.loc[
                    freqs['model.num'] == models.loc[models['patient'].str.contains(patientid), 'model'].values[0], :]
            ...
            if group_name not in masksample:
                ...
                if group_name.startswith("p"):
                    ...
                    if samplenum.isnumeric():
                        if group_name[0] + str(int(samplenum) - 1) in phases or data['phase'].str.match(
                                "^p{1}[A-Z]$") is not None:
                            if int(samplenum) > 1:
                ...
                if group_name.startswith("r"):
                    ...
                    if "i" not in list(data['phase'].str[0]) or "p" not in data['phase'].str[0]:
                        ...
                        left = 900
                    if samplenum.isnumeric():
                        if group_name[0] + str(int(samplenum) - 1) or data['phase'].str.match(
                                "^r{1}[A-Z]$") is not None:
                            if int(samplenum) > 1:
                                left = left + (int(samplenum) - 1) * 200
                                top = top - 210
...
if __name__ == "__main__":
    ...
    preproc_files = [os.path.join(dp, f) for dp, dn, filenames in os.walk(clonevol_preproc_data_path) for f in filenames
                     if f.endswith('.csv')]
